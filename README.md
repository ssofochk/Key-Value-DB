...
#### Политики вытеснения
- LRU
Вытесняет запись, к которой дольше всего не обращались.
...
LRU LFU могут иметь высокий hit ratio и throughput, но требуют хранения дополнительных данных, поэтому обычно проигрывают в этом параметре random и noeviction.
...
Random может вытеснять популярные ключи, его hit ratio зависит от соотношения количества горячих ключей и ключей в кэше -- чем больше вероятность вытеснить горячий ключ, тем больше будет eviction. Ожидается, что LRU и LFU будeт работать быстро и покажут высокий hit ratio. Результат работы noeviction будет зависеть от того, попадут ли популярные ключи в кэш. Если да, то Noeviction чуть уступит LRU LFU, но все еще даст высокий hit ratio и thoughput.
